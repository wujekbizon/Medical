//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Medical.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class Karetka
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2214:DoNotCallOverridableMethodsInConstructors")]
        public Karetka()
        {
            this.HistoriaNapraw = new HashSet<HistoriaNapraw>();
            this.KosztUtrzymania = new HashSet<KosztUtrzymania>();
            this.UdzielonaPomoc = new HashSet<UdzielonaPomoc>();
            this.ZlecenieWyjazdu = new HashSet<ZlecenieWyjazdu>();
            this.ZespolRatunkowy = new HashSet<ZespolRatunkowy>();
        }
    
        public int IdKaretki { get; set; }
        public string NumerRejestracyjny { get; set; }
        public string TypKaretki { get; set; }
        public int RokProdukcji { get; set; }
        public string Status { get; set; }
        public int PrzebiegKm { get; set; }
        public Nullable<System.DateTime> DataOstatniegoPrzegladu { get; set; }
        public Nullable<System.DateTime> DataWaznosciUbezpieczenia { get; set; }
        public Nullable<decimal> PojemnoscSilnika { get; set; }
        public Nullable<int> MocSilnikaKM { get; set; }
        public string NumerVIN { get; set; }
        public string TypPaliwa { get; set; }
        public string Kolor { get; set; }
        public Nullable<int> LiczbaMiejsc { get; set; }
        public Nullable<System.DateTime> DataNabycia { get; set; }
        public Nullable<int> SzacowanaZywotnoscLat { get; set; }
        public int IdPlacowki { get; set; }
        public bool CzyAktywny { get; set; }
        public System.DateTime KiedyDodal { get; set; }
        public string KtoDodal { get; set; }
        public Nullable<System.DateTime> KiedyModyfikowal { get; set; }
        public string KtoModyfikowal { get; set; }
        public Nullable<System.DateTime> KiedyWykasowal { get; set; }
        public string KtoWykasowal { get; set; }
        public int WersjaDanych { get; set; }
    
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<HistoriaNapraw> HistoriaNapraw { get; set; }
        public virtual Placowka Placowka { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<KosztUtrzymania> KosztUtrzymania { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<UdzielonaPomoc> UdzielonaPomoc { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<ZlecenieWyjazdu> ZlecenieWyjazdu { get; set; }
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2227:CollectionPropertiesShouldBeReadOnly")]
        public virtual ICollection<ZespolRatunkowy> ZespolRatunkowy { get; set; }
    }
}
